// Arquivo: client/src/components/ReviewCard.tsx
        ...
        <div className="flex items-start space-x-4">
          ...
          <div className="flex-1">
            <div className="flex items-start justify-between gap-4 mb-2">
                {/* --- MUDANÇA 2: LAYOUT ROBUSTO DO CABEÇALHO --- */}
                <div className="flex-1 min-w-0">
                    <h4 className="font-medium text-gray-900 truncate">{user?.name || 'Usuário'}</h4>
                    {showRestaurantName && restaurant && (
                        <p className="text-sm text-gray-600 truncate">
                            avaliou o <Link href={`/restaurant/${restaurant.id}`}><a className="font-medium text-orange-600 hover:underline">{restaurant.name}</a></Link>
                        </p>
                    )}
                </div>
            ...
            <div className="flex items-center space-x-4 text-sm text-gray-500 border-b pb-3 mb-3">
              <span>Visita em: {new Date(review.visitDate).toLocaleDateString('pt-BR')}</span>
            </div>
